...
class PackageBottomSheet extends HookConsumerWidget {
  final PackageModel? package;
  const PackageBottomSheet({super.key, this.package});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final isEditing = package != null;
    final nameController = useTextEditingController(text: package?.name);
    final descController = useTextEditingController(text: package?.description);
    final priceController = useTextEditingController(
      text: package?.price.toString(),
    );
    final selectedCourseId = useState<String?>(package?.courseId);
    final selectedOrderTypeId = useState<String?>(package?.orderTypeId);
    final selectedMenuItems = useState<List<String>>(package?.menuItems ?? []);
    ...
    final packageState = ref.watch(packageControllerProvider);
    final isLoading = packageState.status == PackageActionStatus.loading;

    ref.listen<PackageState>(packageControllerProvider, (prev, next) {
      if (next.status == PackageActionStatus.success) {
        if (context.mounted) Navigator.of(context).pop();
        showSnackBar(context, 'Package saved!');
      }
      if (next.status == PackageActionStatus.error) {
        showSnackBar(
          context,
    ...
    void submit() {
      if (formKey.currentState?.validate() ?? false) {
        ...
        final controller = ref.read(packageControllerProvider.notifier);
